...
static void term_on_key(window_t *win, char key) {
    ...
    if (key == '\n') {
        ...
        if (cmd_len > 0) {
            ...
            } else if (k_strcmp(cmd_buf, "doom") == 0) {
#ifdef DOOM_ENABLED
                gui_print("Lanzando DOOM...\n", THEME_TERM_SUCCESS);
                doom_launch("/doom.wad");
#else
                gui_print("DOOM no compilado. Usa 'make doom' y 'make iso-doom'\n", THEME_TERM_ERROR);
#endif
            } else if (k_strncmp(cmd_buf, "doom ", 5) == 0) {
#ifdef DOOM_ENABLED
                gui_print("Lanzando DOOM con WAD personalizado...\n", THEME_TERM_SUCCESS);
                doom_launch(cmd_buf + 5);
#else
                gui_print("DOOM no compilado.\n", THEME_TERM_ERROR);
#endif
            } else if (k_strcmp(cmd_buf, "ls") == 0 ||
                       k_strcmp(cmd_buf, "dir") == 0) {
                extern int fat32_ready;
                if (fat32_ready) {
                    gui_print("Listado del directorio raiz:\n", THEME_TERM_TEXT);
                    extern int fat32_list_dir(const char*, void*, void*);
                    // Stage 4: implementar listado completo
                    gui_print("  (FAT32 disponible - Stage 4 tendra 'ls' completo)\n", THEME_TERM_TEXT);
                } else {
                    gui_print("Sin disco FAT32 detectado\n", THEME_TERM_ERROR);
                }
            } else if (k_strcmp(cmd_buf, "mem") == 0) {
                extern size_t mm_free_bytes(void);
                extern size_t mm_used_bytes(void);
                char mbuf[64];
                k_sprintf(mbuf, "  Libre: %d KB | Usado: %d KB\n",
                    (int)(mm_free_bytes()/1024), (int)(mm_used_bytes()/1024));
                gui_print(mbuf, THEME_TERM_SUCCESS);
            } else if (k_strcmp(cmd_buf, "help") == 0 ||
                       k_strcmp(cmd_buf, "?") == 0) {
                gui_print("Comandos disponibles:\n", THEME_TERM_PROMPT);
                gui_print("  clear/cls  - Limpiar terminal\n", THEME_TERM_TEXT);
                gui_print("  ver        - Version del sistema\n", THEME_TERM_TEXT);
                gui_print("  mem        - Uso de memoria\n", THEME_TERM_TEXT);
                gui_print("  ls / dir   - Listar archivos\n", THEME_TERM_TEXT);
                gui_print("  doom       - Iniciar DOOM\n", THEME_TERM_TEXT);
                gui_print("  doom <wad> - DOOM con WAD especifico\n", THEME_TERM_TEXT);
                gui_print("  reboot     - Reiniciar\n", THEME_TERM_TEXT);
                gui_print("  about      - Acerca de XP7 OS\n", THEME_TERM_TEXT);
            } else {
